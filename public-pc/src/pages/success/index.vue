  ...
  <div class="success">
    ...
    <div v-if="type===2" class="text"> {{$route.query.info}}</div>
     ...
     v-if="type===2&&$route.query.id"
      ...
      <div  v-if="type===3" class="text">您的在线申诉已成功提交，待工作人员进行处理核实！</div>
      <div  v-if="type===3" class="complain">{{$route.query.info}}</div>
      <div  v-if="type===1" class="text">提交成功！</div>
      <div v-if="type===1" class="register">{{$route.query.info}}您的选民信息已成功提交，待系统审核！后续可通过<span class="search" @click="jup">[在线查询]</span>及时了解进度</div>
  </div>
</template>
